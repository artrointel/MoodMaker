...
class CRTRenderer(bufferFromImage: Buffer, width: Int, height: Int)
    ...
    private lateinit var uFadeDirX: Uniform
    private lateinit var uFadeDirY: Uniform
    ...
    private var fadeDirX: Int = 0
    private var fadeDirY: Int = 1

    fun setAlpha(alpha: Float) {
        this.alpha = alpha
    }

    fun setFadeDirection(x: Int, y: Int) {
        this.fadeDirX = x
        this.fadeDirY = y
    }
    ...
    override fun onInitializeGLObjects(): Array<IGLObject> {
        val vShader = Shader(
            Shader.TYPE.VERTEX,
            Assets.getShaderString("transition_crt.vsh.glsl"))
        val fShader = Shader(
            Shader.TYPE.FRAGMENT,
            Assets.getShaderString("transition_crt.fsh.glsl"))
        ...
        uFadeDirX = Uniform(program, DataType.INT, "uFadeDirX", 1).set(0)
        uFadeDirY = Uniform(program, DataType.INT, "uFadeDirY", 1).set(0)
        ...
        return arrayOf(program, uProjMatrix, uModelMatrix, textureSet, uAlpha,
            uFadeDirX, uFadeDirY, uProgress, uResolution, attrSet)
    }
    ...
    override fun onGLObjectUpdated() {
        uAlpha.set(alpha)
        uFadeDirX.set(fadeDirX)
        uFadeDirY.set(fadeDirY)
