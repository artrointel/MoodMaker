...
class WarpTransitionRenderer(bufferFromImage: Buffer, bufferFromImage2: Buffer, width: Int, height: Int)
    ...
    override fun onInitializeGLObjects(): Array<IGLObject> {
        val vShader = Shader(
            Shader.TYPE.VERTEX,
            Assets.getShaderString("transition_warp.vsh.glsl"))
        val fShader = Shader(
            Shader.TYPE.FRAGMENT,
            Assets.getShaderString("transition_warp.fsh.glsl"))
        ...
        return arrayOf(program, uProjMatrix, uModelMatrix, textureSet, uAlpha, uProgress, attrSet)
    }
